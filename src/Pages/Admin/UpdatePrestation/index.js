...
const UpdatePrestation = ({ prestations }) => {

  const { id } = useParams()
  const presation = prestations.find((pre) => pre.id === id)
  ...
  const [imagePrev, setImagePrev] = useState(presation?.image)
  const [prestation, setPrestation] = useState(presation)
  ...
  return (
    <section id="createPrestation">
      <header>
        <button className='back' onClick={() => navigate('/admin')}>Retour</button>
        <button className='add' onClick={() => updateData(prestation)}>Sauvgarder</button>
      </header>
...
export default UpdatePrestation